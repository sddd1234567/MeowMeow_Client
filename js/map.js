...
function initNewHelpMap() {    
    ...
    if(positionCache.lat == undefined)
    {
        navigator.geolocation.getCurrentPosition(function (position) {
            ...
            nowClickedPosition = { lat: position.coords.latitude, lng: position.coords.longitude };
            var map;
            ...
            // });
            console.log('https://api.opencagedata.com/geocode/v1/json?q=' + position.coords.latitude + '+' + position.coords.longitude + '&key=fec7294f81724e4eb96707e395b4ef03');
            $.get('https://api.opencagedata.com/geocode/v1/json?q=' + position.coords.latitude + '+' + position.coords.longitude + '&key=fec7294f81724e4eb96707e395b4ef03', function (data) {
                ...
                if (data.results[0].components.state_district != undefined)
                    nowClickedPositionDistrict = data.results[0].components.state_district;
                ...
                document.querySelector("[data-page='new-help'] #position").value = nowClickedPositionName;
            });
            ...
            function onMapClick(e) {
                ...
                $.get('https://api.opencagedata.com/geocode/v1/json?q=' + e.latlng.lat + '+' + e.latlng.lng + '&key=fec7294f81724e4eb96707e395b4ef03', function (data) {
                    nowClickedPositionName = data.results[0].formatted;
                    console.log(data.results[0].formatted);
                    if (data.results[0].components.state_district != undefined)
                        nowClickedPositionDistrict = data.results[0].components.state_district;
    ...
    else
    {
        ...
        //     document.querySelector("[data-page='new-help'] #position").value = data.display_name;
        // });
        console.log('https://api.opencagedata.com/geocode/v1/json?q=' + positionCache.lat + '+' + positionCache.lng + '&key=fec7294f81724e4eb96707e395b4ef03');
        $.get('https://api.opencagedata.com/geocode/v1/json?q=' + positionCache.lat + '+' + positionCache.lng + '&key=fec7294f81724e4eb96707e395b4ef03', function (data) {
            nowClickedPositionName = data.results[0].formatted;
            console.log(data.results[0].formatted);
            if (data.results[0].components.state_district != undefined)
                nowClickedPositionDistrict = data.results[0].components.state_district;
...
function OpenGoogleMapApp(lat,lng){
    window.open("geo:" + lat + "," + lng + "?q="+lat+","+lng+"(受傷地點)", "_system");
}