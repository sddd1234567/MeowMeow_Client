...
app.on("tabShow", function(newTableElement, tabRoute) {
  console.log("switch to ", newTableElement.id);
  switch (newTableElement.id) {
    case "forum-tab":
      ReloadAllForumList();
      break;
    case "pic-share-tab":
      ...
      break;
    case "help-tab":
      console.log("start load help tab");
      loadHelpPage(-1, true);
      break;
...
function onBackKeyDown() {
  if (app.views.current.router.history.length > 1)
    ...
    if (isOpeningExitConfirm) return;
    else isOpeningExitConfirm = true;
    app.dialog.confirm(
      "確定要離開MeowMeow嗎？",
      function() {
      ...
      for (i in list) {
        if (!helpArticleNameCache.includes(i)) {
          ...
          if (positionCache.lat != undefined) {
            if (list[i].poster == userInfo.uid) continue;
            ...
            if (distance <= 0.5) {
              ...
              app.notification
                .create({
                  closeTimeout: 3000,
                  icon:
                    '<img src="img/app_icon.png" style="width:13px;height:13px;"></img>',
                  title: "MeowMeow",
                  titleRightText: "now",
                  subtitle: "有貓貓受傷了！",
                  text:
                    "在約" + distance + "公尺處有被發現的受傷貓貓，去看看吧！",
                  closeOnClick: true,
                  on: {
                    click: function() {
                      app.views.current.router.navigate(
                        "/help-content/?index=" + i + ""
                      );
                    }
                  }
                })
...
$$(document).on("page:init", function(e) {
  ...
  console.log("switch to ", nowPageName);
  switch (nowPageName) {
    ...
    case "new-pic-share-post":
      document.querySelector("[data-page='new-pic-share'] .user-icon").src =
        userInfo.icon;
      document.querySelector(
        "[data-page='new-pic-share'] .poster-name"
    ...
    case "new-forum-article":
      document.querySelector("[data-page='new-forum-article'] .user-icon").src =
        userInfo.icon;
      document.querySelector(
        "[data-page='new-forum-article'] .poster-name"
      ).innerText = userInfo.name;
      break;
    case "help-content":
      ...
      break;
    case "new-help":
      document.querySelector("[data-page='new-help'] .user-icon").src =
        userInfo.icon;
      document.querySelector("[data-page='new-help'] .poster-name").innerText =
        userInfo.name;
      var emergency_rate_el = document.querySelectorAll(
        '[data-page="new-help"] .emergency-rate'
      );
      for (var i = 0; i < emergency_rate_el.length; i++) {
        emergency_rate_el[i].setAttribute(
          "onclick",
          "SetEmergencyRate(" + i + ")"
        );
      }
...
var hospital_ptr = app.ptr.create(
  '[data-page="noob"] #tab-hospital.ptr-content'
);
var $hospitalPtrContent = $$('[data-page="noob"] #tab-hospital.ptr-content');
$hospitalPtrContent.on("ptr:refresh", function(e) {
  console.log("isloadinghospital=", isLoadingHospital);
  if (!isLoadingHospital) {
    LoadHospitalPage("", 0, false, true);
  } else app.ptr.done($hospitalPtrContent);
});
...
var myForum_ptr = app.ptr.create(
  '[data-page="my-post"] #tab-my-forum.ptr-content'
);
...
var myHelp_ptr = app.ptr.create(
  '[data-page="my-post"] #tab-my-help.ptr-content'
);
...
function loadPicSharePage(timestamp, isReload) {
  ...
  GetPicShareList(timestamp, function(list) {
    if (isReload) $$('[data-page="pic-share"] .page-content').scrollTop(0, 100);
...
function loadForumPage(classify, isReload) {
  ...
  GetForumArticleList(classify, function(list) {
    if (isReload)
      $$('[data-page="forum"] #tab-' + classify + ".page-content").scrollTop(
        0,
        100
      );
    for (i in list) console.log(list[i].title);
...
function loadHelpPage(timestamp, isReload) {
  ...
  GetHelpList(timestamp, function(list) {
    if (isReload) $$('[data-page="help"] .page-content').scrollTop(0, 100);
    ...
    $.get(
      ...
      function(data) {
        ...
        for (var i = 0; i < tutorialArticle.length; i++) {
          console.log(i);
          var container = new DOMParser().parseFromString(
            tutorialListItem,
            "text/html"
          );
          container.querySelector(".tutorial-link").href =
            "/noob-tutorial-content/?index=" + i;
          container.querySelector(".tutorial-list-title").innerText =
            tutorialArticle[i].title;
          str = str + container.querySelector(".tutorial-link").outerHTML;
        }
        document.querySelector(
          '[data-page = "noob"] .noob-tutorial-list'
        ).innerHTML = str;
      },
      "json"
    );
...
function ShowDonateRecords(list) {
  ...
  for (i in list) {
    var container = new DOMParser().parseFromString(
      donateRecordListItem,
      "text/html"
    );
    container.querySelector(".donate-records-list-title").innerText =
      list[i].itemName;
    if (list[i].finishPay.toString() == "true")
      ...
      if (t < getNowTimeStamp())
        ...
        container.querySelector(".donate-content-link").href =
          "/donate-payment-content/?index=" + i;
      }
    }
    container.querySelector(".donate-records-list-amount").innerText =
      "捐款金額：" + list[i].totalAmount;
    str += container.querySelector(".donate-content-link").outerHTML;
  }
  document.querySelector(
    '[data-page="donate-records"] .donate-records-list'
  ).innerHTML = str;
}
...
app.infiniteScroll.create(
  $$('[data-page="pic-share"] .page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="forum"] #tab-0.page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="forum"] #tab-1.page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="forum"] #tab-2.page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="forum"] #tab-3.page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="help"] .page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="fundraising"] .page-content.infinite-scroll-content')
);
app.infiniteScroll.create(
  $$('[data-page="noob"] #tab-hospital.page-content.infinite-scroll-content')
);

$$('[data-page="pic-share"] .infinite-scroll-content').on(
  "infinite",
  function() {
...
$$('[data-page="fundraising"] .infinite-scroll-content').on(
  "infinite",
  function() {
...
$$('[data-page="noob"] #tab-hospital.infinite-scroll-content').on(
  "infinite",
  function() {
...
function showPicShareList(list, isReload) {
  if (list == undefined || list == null) {
    ...
    app.infiniteScroll.destroy(
      '[data-page="pic-share"] .infinite-scroll-content'
    );
    document.querySelector(
      "[data-page='pic-share'] .infinite-scroll-preloader"
    ).style.display = "none";
    app.ptr.done($fundraisingPtrContent);
  ...
  if (picShareItemStr == "") {
    $.get("pages/elements/pic-share-item.html", function(data) {
      ...
      for (i in list) {
        ...
        container.querySelector(".user-link").href =
          "/chatroom-content/?index=" + list[i].poster;
        container.querySelector(".pic-share-content-text").innerText =
          list[i].content;
        ...
        if (list[i].likes != undefined) {
          ...
          if (list[i].likes[userInfo.uid] != null) {
            container
              ...
              .setAttribute("src", "img/liked.png");
            container
              .querySelector("#like-link")
              .setAttribute(
                "onclick",
                "OnPicShareCancelLikePressed(this, '" + i + "')"
              );
          } else {
            container
              .querySelector("#like-icon")
        ...
        container.querySelectorAll(
          ".article-footer-count-text"
        )[0].innerText = likes;
        container.querySelectorAll(
          ".article-footer-count-text"
        )[1].innerText = comments;
        container.querySelector(".pic-container").style.backgroundImage =
          "url(" + list[i].img + ")";
        container.querySelector(".pic-container").dataset.background =
          list[i].img;
        container.querySelector(".pic-container").id = "pic-share-img-" + i;
        container.querySelector(".pic-post-time").innerText = timedifference(
          list[i].time,
          nowTimestamp
        );
        ...
        container.querySelector(".content-link").href =
          "/pic-share-content/?index=" + i;
        str = container.querySelector(".pic-item").outerHTML + str;
      }
      if (isReload) $(".pic-share-list").html(str);
      else $(".pic-share-list").append(str);
      ...
      for (i in list) {
        ...
        document
          .querySelector("#pic-share-img-" + i)
          .setAttribute("onclick", 'OpenPhotoBrowser("' + i + '")');
        GetUserInfo(list[i].poster, function(data, uid) {
          // console.log("aaaaaaaa");
          var iconElement = document.querySelector(
            "[data-page='pic-share'] #" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            "[data-page='pic-share'] #" + uid + "-name"
          );
    ...
    for (i in list) {
      ...
      if (list[i].likes != undefined) {
        ...
        container
          .querySelector("#like-link")
          .setAttribute("onclick", "OnPicShareLikePressed(this, '" + i + "')");
      }
      if (list[i].comments != undefined)
        comments = Object.getOwnPropertyNames(list[i].comments).length;
    ...
    if (isReload) $(".pic-share-list").html("");
  ...
  if (picShareItemStr == "") {
    $.get("pages/elements/pic-share-item.html", function(data) {
      ...
      for (i in list) {
        ...
        photoBrowsers[i] = myPhotoBrowserDark;
        document
          .querySelector("[data-page='my-post'] #pic-share-img-" + i)
          .setAttribute("onclick", 'OpenPhotoBrowser("' + i + '")');
      }
    });
  } else {
...
function LoadPicShareContent(index) {
  GetPicShareArticle(index, function(article) {
    ...
    if (article.likes != undefined) {
      ...
      if (article.likes[userInfo.uid] != null) {
        ...
        container
          ...
          .setAttribute(
            "onclick",
            "OnPicShareCancelLikePressed(this, '" + index + "')"
          );
      } else {
        container
          .querySelector("#like-icon")
        ...
        container
          ...
          .setAttribute(
            "onclick",
            "OnPicShareLikePressed(this, '" + index + "')"
          );
      }
    } else {
      console.log("add like listener");
    ...
    if (commentList != undefined) ShowPicShareComments(commentList);
    ...
    GetUserInfo(article.poster, function(data) {
      // console.log("aaaaaaaa");
      $(".page[data-page='pic-share-content'] .pic-item").html(
        container.querySelector(".pic-item").innerHTML
      );
      document.querySelector(
        '[data-page = "pic-share-content"] .new-comment-item .user-icon'
      ).src = userInfo.icon;
      var myPhotoBrowserDark = app.photoBrowser.create({
        photos: [article.img],
        theme: "dark"
      ...
      document.querySelector(
        "[data-page='pic-share-content'] .pic-container"
      ).onclick = function() {
        myPhotoBrowserDark.openPopup();
      };
      var iconElement = document.querySelector(
        ".page[data-page='pic-share-content'] #" + article.poster + "-icon"
      );
      var nameElement = document.querySelector(
        ".page[data-page='pic-share-content'] #" + article.poster + "-name"
      );
  ...
  if (commentItemStr == "") {
    $.get("pages/elements/comment-item.html", function(data) {
      ...
      for (i in commentList) {
        ...
        var commentContainer = new DOMParser().parseFromString(
          commentItemStr,
          "text/html"
        );
        commentContainer.querySelector(".post-time").innerText = timedifference(
          commentList[i].time,
          nowTimestamp
        );
        commentContainer.querySelector(".poster-name").id =
          "comment-" + comment_poster + "-name";
        commentContainer.querySelector(".user-link").href =
          "/chatroom-content/?index=" + commentList[i].poster;
        commentContainer.querySelector(
          ".comment-content"
        ).innerText = comment_content;
        commentContainer.querySelector(".user-icon").id =
          "comment-" + comment_poster + "-icon";
        $(".page[data-page='pic-share-content'] .pic-share-comments").append(
          commentContainer.querySelector(".comment-item").outerHTML
        );
        GetUserInfo(comment_poster, function(data, uid) {
          var iconElement = document.querySelector(
            ".page[data-page='pic-share-content'] #comment-" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            ".page[data-page='pic-share-content'] #comment-" + uid + "-name"
          );
...
function showForumArticleList(list, classify, isReload) {
  if (list == undefined || list == null) {
    ...
    app.infiniteScroll.destroy(
      '[data-page="forum"] #tab-' + classify + ".infinite-scroll-content"
    );
    app.ptr.done($forumTab0PtrContent);
    app.ptr.done($forumTab1PtrContent);
    app.ptr.done($forumTab2PtrContent);
  ...
  if (forumListItemStr == "") {
    $.get("pages/elements/forum-article-list-item.html", function(data) {
      ...
      for (i in list) {
        ...
        container.querySelector(".forum-article-list-item-title").innerText =
          list[i].title;
        ...
        container.querySelector(
          ".forum-article-comment-count"
        ).innerText = likes;

        container.querySelector(".forum-article-content-link").href =
          "/forum-article-content/?index=" + i;

        str =
          container.querySelector(".forum-article-content-link").outerHTML +
          str;
      }

      if (isReload) $("#tab-" + classify + " .forum-article-list").html(str);
      else $("tab-" + classify + " .forum-article-list").append(str);
      isLoadingForum[classify] = false;
    });
  } else {
    var str = "";
    ...
    for (i in list) {
      ...
      str =
        container.querySelector(".forum-article-content-link").outerHTML + str;
    }
    if (isReload) $("#tab-" + classify + " .forum-article-list").html(str);
    else $("tab-" + classify + " .forum-article-list").append(str);
    console.log(str);
...
function LoadForumArticleContent(index) {
  GetForumArticle(index, function(article) {
    document.querySelector(
      "[data-page='forum-article-content'] .navbar .title"
    ...
    container.querySelector(".poster-name").id = article.poster + "-name";
    container.querySelector(".user-link").href =
      "/chatroom-content/?index=" + article.poster;
    container.querySelector(".classify").innerText = GetForumClassifyName(
      article.classify
    );
    ...
    if (article.img != undefined) {
      if (article.img != "") {
        container.querySelector(
          ".forum-main .pic-container"
        ).style.backgroundImage = "url(" + article.img + ")";
        container.querySelector(".forum-main .pic-container").style.display =
          "block";
      }
    }
    console.log("finish get article");
    ...
    if (article.likes != undefined) {
      ...
      if (article.likes[userInfo.uid] != null) {
        ...
        container
          ...
          .setAttribute(
            "onclick",
            "OnForumCancelLikePressed(this, '" + index + "')"
          );
      } else {
        container
          .querySelector("#like-icon")
        ...
        container
          .querySelector("#like-link")
          .setAttribute("onclick", "OnForumLikePressed(this, '" + index + "')");
      }
    } else {
      console.log("add like listener");
      container.querySelector("#like-icon").setAttribute("src", "img/like.png");
    ...
    var commentList = article.comments;
    if (commentList != undefined) ShowForumComments(commentList);

    container.querySelector(".forum-post-time").innerText = timedifference(
      article.time,
      nowTimestamp
    );
    container.querySelectorAll(
    ...
    GetUserInfo(article.poster, function(data) {
      $(".page[data-page='forum-article-content'] .forum-item").html(
        container.querySelector(".forum-item").innerHTML
      );

      document.querySelector(
        '[data-page = "forum-article-content"] .new-comment-item .user-icon'
      ).src = userInfo.icon;
      var myPhotoBrowserDark = app.photoBrowser.create({
        photos: [article.img],
        theme: "dark"
      ...
      document.querySelector(
        "[data-page='forum-article-content'] .pic-container"
      ).onclick = function() {
        myPhotoBrowserDark.openPopup();
      };
      var iconElement = document.querySelector(
        "[data-page='forum-article-content'] #" + article.poster + "-icon"
      );
      var nameElement = document.querySelector(
        "[data-page='forum-article-content'] #" + article.poster + "-name"
      );
  ...
  if (commentItemStr == "") {
    $.get("pages/elements/comment-item.html", function(data) {
      ...
      for (i in commentList) {
        ...
        $(".page[data-page='forum-article-content'] .forum-comments").append(
          commentContainer.querySelector(".comment-item").outerHTML
        );
        GetUserInfo(comment_poster, function(data, uid) {
          var iconElement = document.querySelector(
            "[data-page='forum-article-content'] #" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            "[data-page='forum-article-content'] #" + uid + "-name"
          );
          if (iconElement != null) {
            iconElement.src = data.icon;
            iconElement.id = "";
...
function showHelpList(list, isReload) {
  ...
  if (isReload)
    document.querySelector(
      "[data-page='help'] .infinite-scroll-preloader"
    ).style.display = "block";
  if (list == undefined || list == null) {
    console.log("沒有更多文章");
  ...
  if (helpItemStr == "") {
    $.get("pages/elements/help-item.html", function(data) {
      ...
      for (i in list) {
        ...
        container.querySelector(".user-link").href =
          "/chatroom-content/?index=" + list[i].poster;
        container.querySelector(".help-content-text").innerText =
          list[i].content;
        console.log(list[i]["position"]);
        var position = list[i]["position"];
        container
          .querySelector(".map-link")
          .setAttribute(
            "onclick",
            "OpenGoogleMapApp(" + position["lat"] + "," + position["lng"] + ")"
          );
        // container.querySelector(".pic-container").src = list[i].img;
        ...
        if (list[i].likes != undefined) {
          ...
          if (list[i].likes[userInfo.uid] != null) {
            ...
            container
              ...
              .setAttribute(
                "onclick",
                "OnHelpCancelLikePressed(this, '" + i + "')"
              );
          } else {
            container
              .querySelector("#like-icon")
            ...
            container
              .querySelector("#like-link")
              .setAttribute("onclick", "OnHelpLikePressed(this, '" + i + "')");
          }
        } else {
          console.log("add like listener");
          container
        ...
        container.querySelector(".pic-container").dataset.background =
          list[i].img;
        container.querySelector(".pic-container").id = "pic-share-img-" + i;
        container.querySelector(".help-post-time").innerText = timedifference(
          list[i].time,
          nowTimestamp
        );
        container.querySelector(".location-name").innerText =
          list[i].position_name;

        if (list[i].isSaved != undefined)
          container.querySelector(".chip-label.classify").innerText =
            "已救援完成";
        container.querySelector(".user-icon").id = list[i].poster + "-icon";
      ...
      if (isReload) $(".help-list").html(str);
      else $(".help-list").append(str);
      ...
      for (i in list) {
        ...
        GetUserInfo(list[i].poster, function(data, uid) {
          ...
          var iconElement = document.querySelector(
            "[data-page='help'] #" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            "[data-page='help'] #" + uid + "-name"
          );
          // console.log(iconElement);
          if (iconElement != null) {
            console.log(data.icon);
...
function LoadHelpContent(index) {
  ...
  GetHelpArticle(index, function(article) {
    ...
    if (article.likes != undefined) {
      ...
      if (article.likes[userInfo.uid] != null) {
        ...
        container
          ...
          .setAttribute(
            "onclick",
            "OnHelpCancelLikePressed(this, '" + index + "')"
          );
      } else {
        container
          .querySelector("#like-icon")
        ...
        container
          .querySelector("#like-link")
          .setAttribute("onclick", "OnHelpLikePressed(this, '" + index + "')");
      }
    } else {
      console.log("add like listener");
      container.querySelector("#like-icon").setAttribute("src", "img/like.png");
    ...
    var commentList = article.comments;
    if (commentList != undefined) ShowHelpComments(commentList);
    ...
    GetUserInfo(article.poster, function(data) {
      // console.log("aaaaaaaa");
      $(".page[data-page='help-content'] .help-item").html(
        container.querySelector(".help-item").innerHTML
      );
      document.querySelector(
        '[data-page = "help-content"] .new-comment-item .user-icon'
      ).src = userInfo.icon;
      document
        .querySelector('[data-page="help-content"] .report-help-button')
        .setAttribute("onclick", "ReportHelp('" + articleIndex + "')");
      if (article.isSaved != undefined)
        document.querySelector(
          '[data-page="help-content"] .report-help-button'
        ).style.display = "none";
      var iconElement = document.querySelector(
        ".page[data-page='help-content'] #" + article.poster + "-icon"
      );
      var nameElement = document.querySelector(
        ".page[data-page='help-content'] #" + article.poster + "-name"
      );
      ...
      document.querySelector(
        "[data-page='help-content'] .pic-container"
      ).onclick = function() {
        myPhotoBrowserDark.openPopup();
      };
      console.log("icon: " + data.icon);
  ...
  if (commentItemStr == "") {
    $.get("pages/elements/comment-item.html", function(data) {
      ...
      for (i in commentList) {
        ...
        $(".page[data-page='help-content'] .help-comments").append(
          commentContainer.querySelector(".comment-item").outerHTML
        );
        GetUserInfo(comment_poster, function(data, uid) {
          var iconElement = document.querySelector(
            "[data-page='help-content'] #" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            "[data-page='help-content'] #" + uid + "-name"
          );
          if (iconElement != null) {
            iconElement.src = data.icon;
            iconElement.id = "";
...
function showFundraisingList(list, isReload) {
  ...
  if (list.length < 10) {
    ...
    console.log("沒有更多文章");
    app.infiniteScroll.destroy(
      '[data-page="fundraising"] .infinite-scroll-content'
    );
    document.querySelector(
      "[data-page='fundraising'] .infinite-scroll-preloader"
    ).style.display = "none";
    app.ptr.done($fundraisingPtrContent);
  }
  console.log("show help list");
  ...
  if (fundraisingListItem == "") {
    $.get("pages/elements/fundraising-list-item.html", function(data) {
      ...
      for (i in list) {
        ...
        container.querySelector(".fundraising-list-item-title").innerText =
          list[i].title;
        ...
        var nowDonateRate =
          (parseInt(list[i].now) / parseInt(list[i].target)) * 100;
        container.querySelector(".donate-bar-inner").style.width =
          nowDonateRate + "%";
        // container.querySelector(".pic-container").src = list[i].img;

        container.querySelector(".post-time").innerText = timedifference(
          list[i].time,
          nowTimestamp
        );

        container.querySelector(".content-link").href =
          "/fundraising-content/?index=" + i;
        str = container.querySelector(".content-link").outerHTML + str;
      }
      if (isReload) $(".fundraising-list").html(str);
      else $(".fundraising-list").append(str);
      isLoadingFundraising = false;
    });
  } else {
    var str = "";
...
function LoadFundraisingContent(index) {
  GetFundraisingArticle(index, function(article) {
    var nowTimestamp = getNowTimeStamp();
    document.querySelector(
      '[data-page="fundraising-content"] .fundraising-poster-name'
    ).id = article.poster + "-name";
    document.querySelector(
      '[data-page="fundraising-content"] .fundraising-user-icon'
    ).id = article.poster + "-icon";
    document.querySelector(
      '[data-page="fundraising-content"] .navbar .title'
    ).innerText = article.title;
    document.querySelector(
      '[data-page="fundraising-content"] .fundraising-content-top .title'
    ).innerText = article.title;
    document.querySelector(
      '[data-page="fundraising-content"] .content'
    ).innerText = article.content;
    document.querySelector(
      '[data-page="fundraising-content"] .fundraising-post-time'
    ).innerText = timedifference(article.time, nowTimestamp);

    document.querySelector("#donate-target").innerText =
      "目標金額：$" + article.target;
    document.querySelector("#donate-now").innerText =
      "目前累積金額：$" + article.now;
    document.querySelector("#donate-count").innerText =
      "捐款人數：" + article.nowPeople + "人";
    document.querySelector("#fundraising-state .donate-now").innerText =
      article.now;
    document.querySelector("#fundraising-state .donate-target").innerText =
      article.target;
    var nowDonateRate =
      (parseInt(article.now) / parseInt(article.target)) * 100;
    document.querySelector("#fundraising-state .donate-bar-inner").style.width =
      nowDonateRate + "%";
    document.querySelector("#fundraising-state .donate-link").href =
      "/select-payment/?index=" + index + "&name=" + article.title;
    document.querySelector("#fundraising-info .donate-link").href =
      "/select-payment/?index=" + index + "&name=" + article.title;

    if (article.updates != undefined) {
      for (i in article.updates) {
        var container = new DOMParser().parseFromString(
          fundraisingUpdateListItem,
          "text/html"
        );
        container.querySelector(".update-link").href =
          "/fundraising-update-content/?index=" + index + "&updateIndex=" + i;
        container.querySelector(".post-time").innerText = timedifference(
          article.updates[i].time,
          nowTimestamp
        );
        container.querySelector(".title").innerText = article.updates[i].title;
        container.querySelector(".content").innerHTML =
          article.updates[i].content;

        $("[data-page='fundraising-content'] #fundraising-update").append(
          container.querySelector(".update-link").outerHTML
        );
      }
    }
    ...
    GetMidWayInfo(article.poster, function(data) {
      var iconElement = document.querySelector(
        ".page[data-page='fundraising-content'] #" + article.poster + "-icon"
      );
      var nameElement = document.querySelector(
        ".page[data-page='fundraising-content'] #" + article.poster + "-name"
      );

      if (iconElement != null) iconElement.src = data.icon;
      if (nameElement != null) nameElement.innerText = data.principle_name;

      document.querySelector("#midway-name").innerText =
        "中途之家名稱：" + data.midway_name;
      document.querySelector("#principle-name").innerText =
        "負責人姓名：" + data.principle_name;
      document.querySelector("#address").innerText = "地址：" + data.address;
      document.querySelector("#telphone").innerText =
        "連絡電話：" + data.telephone;
      document.querySelector("#email").innerText = "電子郵件：" + data.email;
      document.querySelector("#visit-time").innerText =
        "可參觀時間：" + data.visit_time;
    });
  });
}
...
function LoadFundraisingUpdateContent(index, updateIndex) {
  GetFundraisingUpdate(index, updateIndex, function(article) {
    var nowTimestamp = getNowTimeStamp();
    document.querySelector(
      '[data-page="fundraising-update-content"] .poster-name'
    ).id = article.poster + "-name";
    document.querySelector(
      '[data-page="fundraising-update-content"] .user-icon'
    ).id = article.poster + "-icon";
    document.querySelector(
      '[data-page="fundraising-update-content"] .navbar .title'
    ).innerText = article.title;
    document.querySelector(
      '[data-page="fundraising-update-content"] .article-title'
    ).innerText = article.title;
    document.querySelector(
      '[data-page="fundraising-update-content"] .fundraising-update-content-text'
    ).innerHTML = article.content;
    document.querySelector(
      '[data-page="fundraising-update-content"] .fundraising-update-post-time'
    ).innerText = timedifference(article.time, nowTimestamp);

    GetMidWayInfo(article.poster, function(data) {
      var iconElement = document.querySelector(
        ".page[data-page='fundraising-update-content'] #" +
          article.poster +
          "-icon"
      );
      var nameElement = document.querySelector(
...
function LoadDonatePaymentContent(index) {
  GetDonatePaymentContent(index, function(value) {
    $('[data-page="donate-payment-content"] .content-block').attr(
      "style",
      "display:block;"
    );
    if (value.paymentMethod == "CVS") {
      ...
      document.querySelector(
        '[data-page="donate-payment-content"] #itemName'
      ).innerText = value.itemName;
      document.querySelector(
        '[data-page="donate-payment-content"] #tradeNo'
      ).innerText = value.tradeNo;
      document.querySelector(
        '[data-page="donate-payment-content"] #expireDate'
      ).innerText = value.expireDate;
      document.querySelector(
        '[data-page="donate-payment-content"] #totalAmount'
      ).innerText = value.totalAmount;
      document.querySelector(
        '[data-page="donate-payment-content"] #paymentNoCVS'
      ).innerText = value.paymentNo;
      var cvstype;
      if (value.CVS_Type == "IBON") cvstype = "7-Eleven";
      else if (value.CVS_Type == "FAMILY") cvstype = "全家";
      else if (value.CVS_Type == "HILIFE") cvstype = "萊爾富";
      else if (value.CVS_Type == "OK") cvstype = "OK超商";

      document.querySelector(
        '[data-page="donate-payment-content"] #CVS_Type'
      ).innerText = cvstype;
      if (value.qr != undefined)
        document.querySelector('[data-page="donate-payment-content"] #qr').src =
          value.qr;
      else
        document.querySelector(
          '[data-page="donate-payment-content"] .qr-container'
        ).style.display = "none";
    } else {
      ...
      document.querySelector(
        '[data-page="donate-payment-content"] #paymentNoATM'
      ).innerText = value.paymentNo;
      document.querySelector(
        '[data-page="donate-payment-content"] #bank'
      ).innerText = value.bank;
    }
  });
}
  ...
  if (chatroomListItem == "") {
    $.get("pages/elements/chatroom-list-item.html", function(data) {
      ...
      for (i in list) {
        ...
        container.querySelector(".chatroom-link").href =
          "/chatroom-content/?index=" + i;
      ...
      for (i in list) {
        GetUserInfo(i, function(data, uid) {
          ...
          var iconElement = document.querySelector(
            ".page[data-page='chatroom'] #" + uid + "-icon"
          );
          var nameElement = document.querySelector(
            ".page[data-page='chatroom'] #" + uid + "-name"
          );
  ...
  GetChatroomMessage(targetUser, function(list) {
    ...
    $.get("pages/elements/chatroom-message-item.html", function(data) {
      ...
      for (i in list) {
        ...
        var align = "left";
        if (list[i].poster == userInfo.uid) align = "right";

        container.querySelector(".chatroom-message-item").className =
          "chatroom-message-item " + align;
        if (userIconCache[list[i].poster] == undefined)
          container.querySelector(".chatroom-user-icon").dataset.name =
            list[i].poster;
        else
          container.querySelector(".chatroom-user-icon").src =
            userIconCache[list[i].poster];
        container.querySelector(".chatroom-message-content").innerText =
          list[i].content;
        str += container.querySelector(".chatroom-message-item").outerHTML;
      }
      document.querySelector(
        '[data-page="chatroom-content"] .message-area'
      ).innerHTML = str;
      $('[data-page="chatroom-content"] .page-content').scrollTop(
        $('[data-page="chatroom-content"] .page-content')[0].scrollHeight
      );
      if (userIconCache[targetUser] == undefined) {
        GetUserInfo(targetUser, function(data) {
          userIconCache[targetUser] = data.icon;
          document.querySelector(
            '[data-page="chatroom-content"] .title'
          ).innerText = "與" + data.name + "的對話";
          var list = document.querySelectorAll(
            'img[data-name = "' + targetUser + '"]'
          );
      ...
      if (userNameCache[targetUser] == undefined) {
        ...
        document.querySelector(
          '[data-page="chatroom-content"] .title'
        ).innerText = "與" + userNameCache[targetUser] + "的對話";
      }
    });
  });
  document
    .querySelector('[data-page="chatroom-content"] .send-button')
    .setAttribute("onclick", "SendChatroomMessage('" + targetUser + "')");
}

function LoadTutorialContent(index) {
  var article = tutorialArticle[index];
  document.querySelector(
    '[data-page="noob-tutorial-content"] .title'
  ).innerText = article.title;
  document.querySelector(
    '[data-page="noob-tutorial-content"] .article-title'
  ).innerText = article.title;
  document.querySelector(
    '[data-page="noob-tutorial-content"] .article-main'
  ).innerHTML = article.content;
}
...
function LoadHospitalPage(params, index, isNight, isReload) {
  ...
  console.log("im loading hospital");
  isLoadingHospital = true;
  if (isReload) nowHospitalIndex = 0;
  if (isReload) {
    if (isNight) {
...
function ShowHospitalList(list, isReload) {
  if (isReload) {
    document.querySelector(
      "[data-page='noob'] #tab-hospital .infinite-scroll-preloader"
    ).style.display = "block";
    document.querySelector(
      '[data-page = "noob"] .noob-hospital-list'
    ).innerHTML = "";
  }
  if (list.length < 20)
    document.querySelector(
    ...
    navigator.geolocation.getCurrentPosition(
      function(position) {
        ...
        for (var i = 0; i < sortList.length; i++) {
          ...
          container.querySelector(".hospital-name").innerText =
            sortList[i]["機構名稱"];
          container.querySelector(".hospital-distance").innerText =
            "距離 " +
            calcCrow(
              positionCache.lat,
              positionCache.lng,
            ...
            "公里";
          container.querySelector(".hospital-address").innerText =
            "地址：" + sortList[i]["機構地址"];
          container
            .querySelector(".hospital-phone-link")
            .setAttribute(
              "onclick",
              'OpenLink("tel:' + sortList[i]["機構電話"] + '")'
            );
          container
            .querySelector(".hospital-locate-link")
            .setAttribute(
              "onclick",
              'OpenLink("geo:' +
                sortList[i].lat +
        ...
        if (isReload)
          ...
          document.querySelector(
            ...
            document.querySelector('[data-page = "noob"] .noob-hospital-list')
              .innerHTML + str;
  ...
  function compare(a, b) {
    if (
      calcCrow(positionCache.lat, positionCache.lng, a.lat, a.lng) <
      calcCrow(positionCache.lat, positionCache.lng, b.lat, b.lng)
    )
      return -1;
    if (
      calcCrow(positionCache.lat, positionCache.lng, a.lat, a.lng) >
...
function OnCiyFilterChanged() {
  ...
  var cityInfo = $.grep(taiwanDistrictJson, function(e) {
    return e.city == city;
  })[0];
  document.querySelector(".district-container").innerHTML = "";
  for (var i = 0; i < cityInfo.area.length; i++) {
    document.querySelector(".district-container").innerHTML +=
      '<p> <input type="checkbox" name="district" id="' +
      cityInfo.area[i]["#text"] +
      '" data-name="' +
      cityInfo.area[i]["#text"] +
      '"/> <label for="' +
      cityInfo.area[i]["#text"] +
...
function ApplyHospitalFilter() {
  ...
  var params = "";
  if (city != "全部縣市") params = "?縣市_like=" + city;
...
function UpdateUserProfile() {
  document.querySelector('[data-page="user-info"] #user-icon').src =
    userInfo.icon;
  document.querySelector('[data-page="user-info"] #user-email').innerText =
    "Email：" + userInfo.email;
  document.querySelector('[data-page="user-info"] #user-name').innerText =
    "會員名稱：" + userInfo.name;
}
...
function GetForumClassifyName(id) {
  if (id == 0) return "情報";
  else if (id == 1) return "提問";
  else if (id == 2) return "送養";
  else return "失蹤";
}
...
function timedifference(date1, date2) {
  ...
  else if (hours > 0) return hours + "小時前";
  else if (minutes > 0) return minutes + "分鐘前";
...
function OpenMidWayApplication() {
  window.open(
    "https://docs.google.com/forms/d/e/1FAIpQLSd2rOIGhgNrQRa_QFv-mrudGuSKC0n65MNPREE677Mj0Pa2_w/viewform?fbclid=IwAR1g9vDJFgTu6n5i-8w710upCEScrgjAobxZeIq653yj59yths-qCrGdmQA",
    "_system"
  );
}
...
var fundraisingView = app.views.create("#fundraising-tab", {
  stackPages: true
});
