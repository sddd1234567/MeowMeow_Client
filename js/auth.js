...
function LoginWithLocalStorage() {
  ...
  if (email != null && password != null) loginWithEmail(email, password);
}
...
firebase.auth().onAuthStateChanged(function(user) {
  ...
  if (user) {
    if (loginScreen != undefined) loginScreen.close();
    if (signUpScreen != undefined) signUpScreen.close();
    ...
    GetUserInfo(userInfo.uid, function(data) {
      ...
      document.querySelector(".panel-left .list li .side-panel-icon").src =
        data.icon;
      document.querySelector("#side-panel-user-name").innerText = data.name;
      document.querySelector(
        '[data-page="user-info"] .user-info-mid #user-name'
      ).innerText = "會員名稱：" + data.name;
      document.querySelector(
        '[data-page="user-info"] .user-info-mid #user-email'
