...
function NewPicShareCommentPost(uid, key, content, callback) {
  ...
  updates[
    "/Users/" + uid + "/Pic-Share-Comments/" + key + "/" + newPostKey
  ] = article;
...
function GetForumArticleList(classify, callback) {
  database
    .ref("Forum")
    .orderByChild("classify")
    .equalTo(classify.toString())
    .once("value", function(snapshot) {
      var list = snapshot.val();
      callback(list);
    });
...
function NewForumCommentPost(uid, key, content, callback) {
  ...
  updates[
    "/Users/" + uid + "/Forum-Comments/" + key + "/" + newPostKey
  ] = article;
...
function NewHelpCommentPost(uid, key, content, callback) {
  ...
  updates[
    "/Users/" + uid + "/Help-Comments/" + key + "/" + newPostKey
  ] = article;
...
function GetFundraisingUpdate(index, updateIndex, callback) {
  database
    .ref("Fundraising/" + index + "/updates/" + updateIndex)
    .once("value", function(snapshot) {
      callback(snapshot.val());
    });
}
...
function GetDonatePaymentContent(index, callback) {
  database
    .ref("Users/" + userInfo.uid + "/donates/" + index)
    .once("value", function(snapshot) {
      callback(snapshot.val());
    });
}
...
function GetDonateRecordsOfUser(uid, callback) {
  database
    .ref("Users/" + userInfo.uid + "/donates")
    .once("value", function(snapshot) {
      callback(snapshot.val());
    });
}
...
function GetChatroomMessage(targetUser, callback) {
  database
    .ref("Chatroom/" + userInfo.uid + "/" + targetUser)
    .once("value", function(snapshot) {
      callback(snapshot.val());
    });
}
...
function SendMessage(targetUser, content, callback) {
  ...
  var newPostKey = firebase
    .database()
    .ref()
    .child("Chatroom/" + userInfo.uid + "/" + targetUser)
    .push().key;
  updates[
    "/Chatroom/" + userInfo.uid + "/" + targetUser + "/" + newPostKey
  ] = messageData;
  updates[
    "/Chatroom/" + targetUser + "/" + userInfo.uid + "/" + newPostKey
  ] = messageData;
...
var jsonServer = "http://3.17.5.241:3001/hospital";
var hospitalCache;
      ...
      cordova.plugins.diagnostic.isGpsLocationEnabled(enabled => {
        ...
        if (!enabled) {
          ...
          navigator.geolocation.getCurrentPosition(function(position) {
            ...
            function compare(a, b) {
              if (
                calcCrow(lat, lng, a.lat, a.lng) <
                calcCrow(lat, lng, b.lat, b.lng)
              )
                return -1;
              if (
                calcCrow(lat, lng, a.lat, a.lng) >
...
var jsonServerNight = "http://3.17.5.241:3001/nighthospital";
function GetNightHospitalList(params, index, callback) {
  console.log(jsonServerNight + params);

  $.get(
    encodeURI(jsonServerNight + params),
    function(data) {
      console.log(data.length);
      cordova.plugins.diagnostic.isGpsLocationEnabled(enabled => {
        console.log(enabled);
